...
void flip_phase_all_ones(QuantumCircuit& qc, std::size_t n) {
    ...
    for (std::size_t j = 0; j + 1 < n; ++j) controls[j] = static_cast<qubit_t>(j);
...
QuantumCircuit xor_oracle(std::size_t n_in, std::size_t n_out,
    ...
    for (std::size_t x = 0; x < in_dim; ++x) {
        ...
        if (fx > out_mask) {
            throw std::invalid_argument("xor_oracle: f(x) exceeds output register");
        }
...
QuantumCircuit phase_oracle(std::size_t n, std::span<const std::uint64_t> marked) {
    ...
    for (std::uint64_t m : marked) {
        if (m >= (std::uint64_t{1} << n)) {
            throw std::invalid_argument("phase_oracle: marked state out of range");
        }
