...
int main() {
    ...
    auto dj_b = deutsch_jozsa(
        4, [](std::uint64_t x) { return std::popcount(x) % 2 == 1; });
    std::cout << "   f(x) = 1       -> "
              << (dj_c.is_constant ? "constant" : "balanced")
              << "\n   f(x) = parity  -> "
              << (dj_b.is_constant ? "constant" : "balanced") << "\n\n";

    std::cout << "2) Bernstein-Vazirani: recover hidden s from f(x) = s.x (1 "
                 "query)\n";
    ...
    auto sm = simon(
        4, [s_simon](std::uint64_t x) { return std::min(x, x ^ s_simon); }, 42);
    ...
    std::cout << "   best state = " << gr.best_state
              << ", P(success) = " << gr.success_probability << " after "
